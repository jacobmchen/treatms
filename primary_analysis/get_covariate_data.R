...
print("baseline covars")
missing <- baseline_covars %>%
    filter(is.na(new.T2.lesions))
print(missing)
...
print("social status")
print(head(social_status))
print(sum(is.na(social_status$sex_at_birth)))

# read the risk strat data, which contains baseline covariate data
risk_strat <- data.frame(read_excel(data_file_name, sheet="risk strat"))

print(head(risk_strat))
print(length(unique(risk_strat$patient_id)))
